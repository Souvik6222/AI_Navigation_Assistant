...
# Architectural Post-Mortem & Changelog (Post-Merge)

This section details every critical pitfall, architectural bottleneck, and crash we encountered while polishing the C++ port, and how we solved them.

## 1. Why Port to C++? (The Python Bottleneck)
**The Trap:** When prototyping in Python with PyTorch, we quickly hit a wall where the pipeline could not exceed 10 FPS, despite having a 20-core / 28-thread CPU.
**The Cause:** Python's Global Interpreter Lock (GIL) forces CPU-bound tasks (like iterating over bounding boxes, calculating depth regions, and drawing annotations) to execute on a single thread. Even with multithreading libraries, the CPU utilization was artificially capped.
**The Solution:** Porting the runtime pipeline to C++ using `ONNX Runtime C++ API` and `OpenCV C++`. This completely bypassed the GIL, allowing true multithreading, zero-copy memory management between OpenCV Mats and ONNX Tensors, and ultimately, paved the way for JNI integration into Android.

## 2. Model Exporting Pitfalls (ONNX)

### A. The `dynamo=True` Crash on MiDaS
**The Trap:** If you try to export the MiDaS depth model to ONNX using modern PyTorch (`torch.onnx.export`), it will fail with a massive internal compiler error.
**The Cause:** PyTorch 2.x defaults to `dynamo=True` (TorchDynamo) for exports. MiDaS contains highly complex, dynamic control flow and unsupported ops that TorchDynamo cannot currently trace cleanly.
**The Solution:** You MUST explicitly force `dynamo=False` during export.
*Code:*
```python
torch.onnx.export(model, dummy_input, "midas.onnx", dynamo=False, ...)
```

## 3. Runtime Crashes & Fixes

### A. MiDaS Depth Estimator Tensor Dimension Crash
**The Trap:** The application would run perfectly, but the exact millisecond a person or chair walked into the frame, it would instantly core dump.
**The Cause:** The C++ code assumed the ONNX model would always return a 4D tensor (`[Batch, Channels, Height, Width]`). However, depending on the PyTorch export settings, MiDaS sometimes returns a squeezed 3D tensor (`[Batch, Height, Width]`). Accessing `output_shape[3]` threw a vector out-of-bounds assertion.
**The Solution:** Implemented dynamic dimension checking in `depth_estimator.cpp`:
```cpp
...
### B. YOLOv8 Tensor Transpose Matrix Trap
**The Trap:** Reading the raw float array from the YOLOv8 ONNX output resulted in garbage bounding boxes scattered randomly across the screen.
**The Cause:** Standard YOLOv8 models output a tensor of shape `[1, 84, 8400]` (Features x Detections). However, standard row-major C++ iteration expects `[1, 8400, 84]` (Detections x Features). 
**The Solution:** We implemented an in-memory transpose matrix in `detector.cpp` to gracefully flip the tensor if `dim1 < dim2`, ensuring row-major parsing works flawlessly regardless of how the YOLO model was exported.

## 4. Extreme CPU Bottlenecks

### A. The Thread-Pool Thrashing Phenomenon (22 FPS -> 5 FPS)
**The Trap:** We have a 28-thread CPU. Logically, telling ONNX Runtime to use all 28 threads should yield maximum FPS. Instead, FPS plummeted to 5 FPS.
**The Cause:** YOLOv8 and MiDaS are relatively small models operating on tiny 256x256 matrices. Distributing such a small workload across 28 threads creates catastrophic synchronization overhead. The CPU cores spent more time waiting on thread locks and thrashing the L3 cache than doing actual matrix math.
**The Solution:** 
We added a `num_threads` setting to `config.yaml` and defaulted it to `4`. Using 2 to 4 threads is the proven "sweet spot" for CPU execution on small neural networks.
```cpp
Ort::SessionOptions opts;
opts.SetIntraOpNumThreads(config.num_threads); // Optimal: 4
```

## 5. Configuration & UX Nightmares

### A. The YAML Indentation Crash
**The Trap:** Editing `config.yaml` caused the app to mysteriously ignore the camera and models, crashing the app.
**The Cause:** `yaml-cpp` is unforgiving. A single missing space indentation (e.g., 1 space instead of 2 spaces on `model_path:`) throws an `end of map not found` exception, killing the entire config parse.
**The Solution:** Always double-check indentation. The code now catches YAML exceptions gracefully.
